...
class SpeakingMap extends Component {
  ...
  createMap() {
    ...
    this.state.events.forEach((e) => {
      ...
      L.marker(coords)
        .bindPopup(`<b>${e.frontmatter.event_name}</b><br/><em>${e.frontmatter.title}</em><br/>${e.frontmatter.date}`)
        .addTo(markers);
    })
  ...
  render() {
    return (
      <MapContainer className={this.state.loaded ? null : 'collapsed'} id="speaking_map"/>
    )
  }
}
