...
import {registerFont, createCanvas, loadImage, Image as CanvasImage} from 'canvas'
...
const TITLE_FONT_FILE = path.join(__dirname, '..', '..', '..', 'public', 'fonts', "AtkinsonHyperlegible-Bold.ttf");
...
function coverBounds(img: CanvasImage, width: number, height: number): [number,number, number, number] {
...
export async function GET({props}: GetParams) {
  registerFont(TITLE_FONT_FILE, { family: TITLE_FONT_FAMILY, weight: TITLE_FONT_WEIGHT })
  ...
  const lines = split(ctx as unknown as CanvasRenderingContext2D, props.title, TITLE_FONT_STYLE, WIDTH - (TEXT_PADDING_X*2), true);
  let textBoxHeight = 48 * lines.length;
  ...
  ctx.beginPath()
  const footerLineY = HEIGHT - (FOOTER_BOTTOM_DISTANCE*2) - FOOTER_FONT_SIZE - FOOTER_LINE_STROKE_SIZE
